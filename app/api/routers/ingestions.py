...
async def create_ingestion(
    ...
    Returns 409 if the identical file was already uploaded for this dataset.
    """
...
@router.get("/api/v1/ingestions/{ingestion_id}/preview", response_model=list[dict[str, Any]])
def preview_ingestion(
    ingestion_id: int,
    rows: int = Query(50, ge=1, le=1000),
    zone: Literal["raw", "clean"] = Query("raw"),
    db: Session = Depends(get_db),
):
    """Preview raw CSV/TXT uploads or the first clean Parquet output for an ingestion."""
    return IngestionService(db).preview(ingestion_id, rows=rows, zone=zone)
